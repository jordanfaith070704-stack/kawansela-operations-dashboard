-- `record_sale` is executable only by the service role (or database owner),
-- but its original human-location check would reject an authenticated server
-- import. This retains member scoping for users and explicitly permits only
-- the already-restricted service role for POS imports.
...
begin
  ...
  for v_pack in
    select li.item_id from public.location_inventory li join public.inventory_items i on i.id=li.item_id
